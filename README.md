...
<!-- results:start -->
Backtested on 38 trades across 10 tickers (2024-2026), buying bullish signals with confidence >= 0.6 at the filing-date close and selling 5 trading days later:

| Metric | Value |
|---|---|
| Trades | 38 |
| Win Rate | 47.4% |
| Avg Return per Trade | -0.7% |
| Total Return | -34.0% |
| SPY Benchmark (same period) | +26.0% |
| Sharpe Ratio | -0.60 |
| Max Drawdown | -60.4% |
...
Latest run: 2026-08-22. Run history: 10 backtests recorded since 2026-05-24, refreshed automatically by the daily data pipeline.
<!-- results:end -->

This section is generated by `scripts/update_readme_results.py` from the same
export that feeds the dashboard, so the numbers here always match the demo.
...
A GitHub Actions workflow (`.github/workflows/daily-data.yml`) runs the full
pipeline nightly on trading days: it restores the database from the committed
dataset, ingests new SEC filings, scores only unanalyzed earnings filings with
the LLM (capped at 10 calls per run), re-runs the backtest when there are new
signals, and commits the refreshed dataset. The push redeploys the dashboard,
so the live demo tracks real filings with no always-on backend.
